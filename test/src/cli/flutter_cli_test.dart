...
void main() {
  group('Flutter', () {
    ...
    group('.test', () {
      ...
      test('throws when pubspec not found (recursive)', () {
        final tempDir = Directory.systemTemp.createTempSync();
        expectLater(
      ...
      test('completes when there is no test directory (recursive)', () async {
        final directory = Directory.systemTemp.createTempSync();
        File(p.join(directory.path, 'pubspec.yaml')).createSync();
        await expectLater(
          Flutter.test(
      ...
      test('runs tests (noisy)', () async {
        ...
        expect(
          ...
          equals([
            ...
            '''\x1B[2K\r00:02 +3 ~2: ...s a really long test name that should get truncated by scalable test''',
